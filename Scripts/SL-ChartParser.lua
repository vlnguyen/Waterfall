...
local GetSimfileChartString = function(SimfileString, StepsType, Difficulty, StepsDescription, Filetype)
	...
	if Filetype == "ssc" then
		...
		for noteData in SimfileString:gmatch("#[Nn][Oo][Tt][Ee][Dd][Aa][Tt][Aa].-#[Nn][Oo][Tt][Ee][Ss]2?:[^;]*") do
			...
			difficulty = OldStyleStringToDifficulty(difficulty)
			difficulty = (difficulty) and ToEnumShortString(difficulty) or "None"
	...
	if (SL[pn].Streams.Filename ~= filename or
		...
		if simfileString then
			...
			if chartString ~= nil and BPMs ~= nil then
				...
				local id = WF.GetStepsID(SONGMAN:GetSongFromSteps(steps), steps)
				local Hash = WF.HashCache[id] or Bin2Hex(CRYPTMAN:SHA1String(chartString..BPMs)):sub(1, 16)
...
-- function to just get the hash, passing in the simfile string
-- use this to generate the hash cache before any players are active etc
GetHashFromSimfileString = function(steps, simfileString, fileType)
	local filename = steps:GetFilename()
	local stepsType = ToEnumShortString( steps:GetStepsType() ):gsub("_", "-"):lower()
	local difficulty = ToEnumShortString( steps:GetDifficulty() )
	local description = steps:GetDescription()
