...
local RecentMods = function(mods)
	...
	local itg = (mods.PreferredGameEnv == "ITG")
	...
	return text, mods.NoteSkin, mods.JudgmentGraphic, itg
end
-- ----------------------------------------------------
...
-- subtitle and avatar path
local Subtitle = function(dir)
	if not dir then return "" end
	local path = dir.."/subtitle.txt"
...
local AvatarPath = function(dir)
	local fb = THEME:GetPathG("", "_profilecard/fallbackav.png")
	if not dir then return fb end
	local path = dir.."/avatar.png"
	if not FILEMAN:DoesFileExist(path) then return fb end
	return path
end
...
for i=1, PROFILEMAN:GetNumLocalProfiles() do
	...
	local mods, noteskin, judgment, itg = RecentMods(userprefs)
